...
app.get('/myinfo', async (req, res) => {
  ...
  if (!data || error) {
    return res.status(400).json({ message: `내 정보 불러오기 실패 : ${id} 해당 유저의 정보가 없습니다.` });
  }
...
app.put('/myinfo/modifiy', async (req, res) => {
  ...
  try {
    ...
    if (!data || error) {
      return res.status(400).json({ message: `내 정보 수정 실패 : ${userData.id} 해당 유저의 정보가 없습니다.` });
    }
    ...
    if (updateError) {
      return res.status(400).json({ message: `내 정보 수정 실패: ${userData.id} 유저 업데이트 오류가 발생했습니다.` });
    }
    return res.status(200).json({ message: '내 정보 수정 성공', data: updatedData });
  } catch (error) {
    return res.status(400).json({ message: `내 정보 수정 실패: ${userData.id} 유저 업데이트 오류가 발생했습니다.` });
  }
});
